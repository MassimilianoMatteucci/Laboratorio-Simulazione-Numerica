...
int main()
{  
  string stato="Gas"; //per indicare in quale stato sto lavorando Solid,Liquid,Gas
  bool restart=false;
  ...
  for(int iblk=1; iblk <= nblk; ++iblk) //Simulation
  {
    ...
    Averages(iblk,stato);   //Print results for current block
  }
...
//update of the histogram of g(r)
      bool find=false;
     // double r,step;
      //r=(double)(i-igofr)*box/((double)nbins*2);
     // step=box/((double)nbins*2);
      int k=igofr;
      while(find==false && k<igofr+nbins) {
        if((double)(k-igofr)*box/((double)nbins*2)<=dr && dr<=(double)(k-igofr)*box/((double)nbins*2)+box/((double)nbins*2)){
          walker[k]+=2.0;
          find=true;
        }
        k++;
...
void Averages(int iblk,string stato) //Print results for current block
{
   ...
   ofstream Gofr, Gave, Epot, Pres;
    ...
    if(stato=="Solid" or stato=="Liquid" or stato=="Gas"){
      ...
      Pres.open("Argon"+stato+"\\ave_pres"+stato+".out",ios::app);
      Gave.open("Argon"+stato+"\\gave"+stato+".out",ios::app);
      Gofr.open("Argon"+stato+"\\gofr"+stato+".out",ios::app);
    }
    else{
...
//g(r)
    ...
    for (int i = igofr; i < igofr+nbins; i++){
      ...
      if(iblk==nblk)
        Gave  << iblk << " " << bin_size/2.+(i-igofr)*bin_size/*r+dr/2.0*/ <<" "<< glob_av[i]/(double)iblk << " "<< Error(glob_av[i],glob_av2[i],iblk) << endl;
    }
